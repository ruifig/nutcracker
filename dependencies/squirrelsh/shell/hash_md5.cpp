...
void MD5Update (MD5Context *ctx, unsigned char const *buf, size_t len)
{
	unsigned t = ctx->bits[0];
	ctx->bits[0] = t + (unsigned(len) << 3);
	if (ctx->bits[0] < t)
		ctx->bits[1]++;
...
void Hash_MD5 (FILE* file, unsigned char* block, unsigned char* hash, SQInteger left)
{
	MD5Context ctx;
	MD5Init(&ctx);
	do
